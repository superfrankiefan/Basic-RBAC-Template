...
public class CommonTreeDTO implements Serializable {
	...
	public CommonTreeDTO(SysResource resource) {
		this.key = resource.getId();
		this.icon = resource.getIcon();
		this.parentId = resource.getParentId();
		this.title = resource.getName();
		this.slotTitle =  resource.getName();
		this.value = resource.getId();
		this.isLeaf = resource.isLeaf();
		this.label = resource.getName();
		if(!resource.isLeaf()) {
			this.children = new ArrayList<CommonTreeDTO>();
		}
	}

	 public CommonTreeDTO(String key, String parentId, String slotTitle, Integer ruleFlag, boolean isLeaf) {
    	this.key = key;
