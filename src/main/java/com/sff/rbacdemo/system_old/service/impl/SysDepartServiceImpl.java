...
import com.sff.rbacdemo.common.utils.FillRuleUtil;
import com.sff.rbacdemo.common.utils.FindsChildrenDeptUtil;
...
import com.sff.rbacdemo.system_old.mapper.SysDepartMapper;
import com.sff.rbacdemo.system_old.mapper.SysUserDepartMapper;
import com.sff.rbacdemo.system_old.mapper.SysUserMapper;
import com.sff.rbacdemo.system_old.service.ISysDepartService;
...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
    ...
    public List<SysDepartTreeDTO> queryMyDeptTreeList(String departIds) {
        ...
        List<SysDepartTreeDTO> listResult = FindsChildrenDeptUtil.wrapTreeDataToTreeList(listDepts);
        return listResult;
    }
...
//	@Cacheable(value = CacheConstant.SYS_DEPARTS_CACHE)
    ...
    public List<SysDepartTreeDTO> queryTreeList() {
        ...
        // 调用wrapTreeDataToTreeList方法生成树状数据
        List<SysDepartTreeDTO> listResult = FindsChildrenDeptUtil.wrapTreeDataToTreeList(list);
        return listResult;
    }

    //	@Cacheable(value = CacheConstant.SYS_DEPART_IDS_CACHE)
    @Override
    public List<SimpleDepartTreeDTO> queryDepartIdTreeList() {
        LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
        query.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
        query.orderByAsc(SysDepart::getDepartOrder);
        List<SysDepart> list = this.list(query);
        // 调用wrapTreeDataToTreeList方法生成树状数据
        List<SimpleDepartTreeDTO> listResult = FindsChildrenDeptUtil.wrapTreeDataToDepartIdTreeList(list);
        return listResult;
    }
    ...
    public void saveDepartData(SysDepart sysDepart, String username) {
        if (sysDepart != null && username != null) {
            ...
            String[] codeArray = (String[]) FillRuleUtil.executeRule(CommonConstant.DEPART, formData);
    ...
    public List<SysDepartTreeDTO> searchBy(String keyWord, String myDeptSearch, String departIds) {
        ...
        if (departList.size() > 0) {
            for (SysDepart depart : departList) {
                model = new SysDepartTreeDTO(depart);
     ...
     */
    @Override
    public List<SysDepartTreeDTO> queryTreeListByPid(String parentId) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            SysDepartTreeDTO treeModel = new SysDepartTreeDTO(depart);
    ...
    private void queryAllParentIdRecursion(String fieldName, String value, JSONObject data) {
        ...
        if (depart != null) {
            ...
            if (ObjectConvertUtils.isNotEmpty(depart.getParentId())) {
    ...
    public SysDepart queryCompByOrgCode(String orgCode) {
        String compyOrgCode = orgCode.substring(0, 3);
