...
import com.sff.rbacdemo.system_old.service.ISysUserDepartService;
...
public class SysUserDepartServiceImpl extends ServiceImpl<SysUserDepartMapper, SysUserDepart> implements ISysUserDepartService {
    @Autowired
    @Qualifier("sysDepartServiceImpl")
     ...
     */
    @Override
    public List<SimpleDepartTreeDTO> queryDepartIdsOfUser(String userId) {
        ...
        try {
            ...
            List<SimpleDepartTreeDTO> depIdModelList = new ArrayList<>();
            ...
            if (userDepList != null && userDepList.size() > 0) {
                ...
                if (depList != null || depList.size() > 0) {
                    for (SysDepart depart : depList) {
                        depIdModelList.add(new SimpleDepartTreeDTO().convertByUserDepart(depart));
                    }
                }
    ...
    public IPage<SysUser> queryDepartUserPageList(String departId, String username, String realname, int pageSize, int pageNo) {
        ...
        if (ObjectConvertUtils.isEmpty(departId)) {
